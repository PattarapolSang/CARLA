...
void  counterEnc3(){
  ...
  if (aState3 != aLastState3){    
    ...
    if (counter3 > 1000) counter3 = 1000;
    
    pseudo_counter3 = counter3/2;
    Serial.print("Position: ");
    Serial.println(pseudo_counter3);
    a3 = calculation2(pseudo_counter3);
    String b3;
    b3 = (String)a3;
    b3.toCharArray(c3, 5);
  } 
...
int calculation2(int pseudo_counter){
  int vol             = 0;
  vol                 = pseudo_counter*10;
...
void  draw() {
  ...
  u8g.drawStr( 0, 48, " Water = ");
  u8g.drawStr( 70, 48, c3 );
  u8g.drawStr( 112, 48, "mL");
  u8g.drawStr( 0, 60, " Total = ");
...
bool  process_checking(){                                                        
  //1.if no tank -> false, 2.set zero tank, 3.close lid check, 4.calculate stepper rev 
  //long measurement;
  //measurement = pulseIn (vibroPin, HIGH); //wait for the pin to get HIGH and returns measurement
  //if (measurement > 1000) return true;
  //else return false;
...
#define PWM_C_PIN           6
//#define ENABLE_MT_C_HPIN    8
//#define ENABLE_MT_C_LPIN    7


#define Flow_Rate_PWM       103            //PWM for Flow Rate 1L/min

unsigned long   Timer_A;
unsigned long   Timer_B;
unsigned long   Timer_C;
unsigned long   Timer_WATER;
unsigned long   TimerOut_Limit  =   60000000; 
...
void  setupMotorDrive(){
    ...
    //pinMode(ENABLE_MT_C_HPIN, OUTPUT);
    //pinMode(ENABLE_MT_C_LPIN, OUTPUT);  
    analogWrite(PWM_C_PIN, 0);  

}

void  StartFilling(){
  bool  FILL_RUN_STATUS   =   true;               //Start Status
  bool  MT_A_Sts          =   true;
  bool  MT_B_Sts          =   true;
  bool  MT_C_Sts          =   true;
  bool  WATER_Sts         =   true;
  ...
  Timer_A                 =   (unsigned long)(a0) * (unsigned long)(60);
  Timer_B                 =   (unsigned long)(a1) * (unsigned long)(60);
  Timer_C                 =   (unsigned long)(a2) * (unsigned long)(60);  
  Timer_WATER             =   (unsigned long)(a3) * (unsigned long)(20);

  //Timer_WATER             =   (unsigned long)(30000);

  while (FILL_RUN_STATUS){
    ...
    if(t_Motor_CurrentTime < (t_Motor_StartTime + Timer_A)){
      analogWrite(PWM_A_PIN, Flow_Rate_PWM);
      Serial.println("Pump A RUNNING ******************************************");     
    }
    ...
    if(t_Motor_CurrentTime < (t_Motor_StartTime + Timer_B)){
      analogWrite(PWM_B_PIN, 129);
      Serial.println("Pump B RUNNING ******************************************");       
    }
    ...
    //////////Motor C Request//////////////////
    if(t_Motor_CurrentTime < (t_Motor_StartTime + Timer_C)){
      analogWrite(PWM_C_PIN, Flow_Rate_PWM); 
      Serial.println("Pump C RUNNING ******************************************");      
    }
    else if(t_Motor_CurrentTime >= (t_Motor_StartTime + Timer_C)){
      analogWrite(PWM_C_PIN ,0);
      Serial.println("Pump C finished ........................................");
      MT_C_Sts    =   false;
    } 

    //////////Yellow Pump Request///////////////////
    if(t_Motor_CurrentTime < (t_Motor_StartTime + Timer_WATER)){
      servo1.writeMicroseconds(1580);
      Serial.println("Water Fill RUNNING ******************************************");  
    }
    else if(t_Motor_CurrentTime >= (t_Motor_StartTime + Timer_WATER)){
      servo1.writeMicroseconds(1000);  
      Serial.println("Water Fill finished ........................................");
      WATER_Sts    =   false;
    } 

    if ((!MT_A_Sts) && (!MT_B_Sts) && (!MT_C_Sts) && (!WATER_Sts)){
      FILL_RUN_STATUS   =   false;
    }
    else if (t_Motor_CurrentTime > (t_Motor_StartTime + TimerOut_Limit)){       ///Time out limit
      FILL_RUN_STATUS   =   false;
    }
    else{}
    ...
    Serial.println("Timer_C = " + String(Timer_C));
    Serial.println("a3 = " + String(a3));
    Serial.println("Timer_WATER = " + String(Timer_WATER));
...
void  setup(void) {
  ...
  if(init_checking() == false){//normally within 3 sec.
    ...
    while(1){// Error Wait for Restart
      buzz_3(); 
      delay(1000);     
    }       
  }
...
void loop(void) {   
  ...
  switch(buttonPushCounter){
    ...
    case 9:
      ...
      /*FILLING PROGRAM ZONE*/

      /*//DEFAULT PROGRAM ENABLE
      delay(3000);
      waterFill(2000);
      buzz_1();
      Chem_A(a0);
